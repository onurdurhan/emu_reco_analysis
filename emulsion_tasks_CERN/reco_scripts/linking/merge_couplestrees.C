...
void merge_couplestrees(const char* eospath, const int brickID, const int platenumber){
 //histo file
 outputfile = new TFile(Form("%i.%i.0.0_merged.cp.root",brickID, platenumber),"RECREATE"); 
 ...
 TString prepath(Form("%s/b%0*i/p%0*i/%i.%i.",eospath,6,brickID, 3,platenumber, brickID,platenumber));
